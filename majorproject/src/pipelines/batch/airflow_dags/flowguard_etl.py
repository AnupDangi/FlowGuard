...
SCRIPTS_DIR  = f"{PROJECT_ROOT}/src/pipelines/batch/python_etl"
VENV_PYTHON  = sys.executable  # Use container's python after installing deps
...
# ═══════════════════════════════════════════════════════════════════════════
# DAG 3: Reconciliation ETL — runs every 15 minutes
# Corrects Redis counters using Bronze source-of-truth (excluding current hour).
# ═══════════════════════════════════════════════════════════════════════════
with DAG(
    dag_id="flowguard_recon_etl",
    description="Bronze source-of-truth reconciliation to Redis (15-min)",
    schedule_interval="*/15 * * * *",
    start_date=datetime(2026, 2, 1),
    catchup=False,
    max_active_runs=1,
    default_args=DEFAULT_ARGS,
    tags=["flowguard", "recon", "etl"],
) as recon_dag:
    run_reconciliation = BashOperator(
        task_id="reconcile_ads_totals",
        bash_command=(
            f"{VENV_PYTHON} "
            f"{PROJECT_ROOT}/src/pipelines/realtime/reconciliation/reconcile_ads_totals.py "
            "--lookback-hours 24"
        ),
        cwd=PROJECT_ROOT,
    )
